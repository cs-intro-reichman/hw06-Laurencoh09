...
public class Runigram {
	...
	public static void main(String[] args) {
		...
		// print(tinypic);
		Color[][] flippedpic = grayScaled(tinypic);
		print(flippedpic);
	...
	public static Color[][] read(String fileName) {
		...
		Color[][] image = new Color[numRows][numColumns];
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		//// Replace the following statement with your code
		Color[][] image_new = new Color[image.length][image[0].length];
		for (int i = 0; i < image_new.length; i++) {
			for (int j = 0; j < image_new[0].length; j++) {
				image_new[i][j] = image[i][image[0].length - 1 - j];
			}
		}
		return image_new;
	}
	...
	public static Color[][] flippedVertically(Color[][] image) {
		...
		for (int i = 0; i < image_new.length; i++) {
			for (int j = 0; j < image_new[0].length; j++) {
				image_new[i][j] = image[image.length - 1 - i][j];
			}
		}
		return image_new;
	}
	...
	public static Color luminance(Color pixel) {
		//// Replace the following statement with your code
		double Red = (0.299 * (double) pixel.getRed());
		double Green = (0.587 * (double) pixel.getGreen());
		double Blue = (0.114 * (double) pixel.getBlue());
		int val = (int) (Red + Green + Blue);
		Color new_pixel = new Color(val, val, val);
		return new_pixel;
	}
	...
	public static Color[][] grayScaled(Color[][] image) {
		...
		for (int i = 0; i < image_new.length; i++) {
			for (int j = 0; j < image_new[0].length; j++) {
				image_new[i][j] = luminance(image[i][j]);
			}
		}
		return image_new;
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		double height_ratio = ((double) original_height / height);
		double width_ratio = ((double) original_width / width);

		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				int scaled_height = (int) (i * height_ratio);
				int scaled_width = (int) (j * width_ratio);
				scaled_image[i][j] = image[scaled_height][scaled_width];
			}
		}
		return scaled_image;
	}
	...
	public static Color[][] blend(Color[][] image1, Color[][] image2, double alpha) {
		Color[][] image_new = new Color[image1.length][image1[0].length];
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		for (int i = 1; i <= n; i++) {
			...
			Color[][] morphed_image = blend(target, source, ratio);
			display(morphed_image);
			StdDraw.pause(500);
		}
	}
